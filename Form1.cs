...
namespace QuestEditor_V2
{
    public partial class Form1 : Form
    {
        ...
        {
            InitializeComponent();
            string path = "Quest.dat";
            if (!File.Exists(path))
            {
                System.Windows.Forms.MessageBox.Show("Missing Serverside Quest.dat");
            }
            else
            {
                using (var stream = System.IO.File.OpenRead("Quest.dat"))
        ...
        private void MonsterCharacter_str()
        {
            ...
            if (File.Exists(path))
            {
                ...
                using (var reader = new BinaryReader(stream))
                {
                    STR_Monsters = new List<STR_Monster>();
                    int _Header = reader.ReadInt32();
                    int _columns = reader.ReadInt32();
                    int _size = reader.ReadInt32();
                    Structure STR = new Structure();
